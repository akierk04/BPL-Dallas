...
function _tiebreakLabel(r, i, rows) {
  // Returns the tiebreaker that separated this row from the one immediately above it
  if (i === 0) return null;
  const prev = rows[i - 1];
        ...
        <tbody>
          ${rows.map((r, i) => {
            ...
            <tr style="border-bottom:0.5px solid var(--border);${i===0?'background:rgba(62,207,142,0.04);':i<3?'background:rgba(240,192,64,0.03);':''}">
              ...
              <td style="padding:10px 10px;font-weight:500;color:var(--text);">${r.captain.team_name || r.captain.name}${qualifier[i]||''}${tbBadge}</td>
